#!/usr/bin/env python3
"""Complete multithreaded ESPN population for all seasons and all tables."""
...
def populate_complete_season(year: int, espn: ESPNClient, db_path: str, db_lock: threading.Lock):
    """Populate ALL data for a season: drivers, teams, races, standings.
    ...
    # Enable WAL mode for better concurrent access and increase timeout
    ...
    try:
        ...
        try:
            ...
            for event in events_data.get('items', []):
                try:
                    ...
                    # Get race results from competitions
                    for comp in event_detail.get('competitions', []):
                        try:
                            comp_ref = comp.get('$ref', '')
                            comp_detail = requests.get(comp_ref).json()
                            ...
                            for competitor in comp_detail.get('competitors', []):
                                try:
                                    ...
                                    # Store race result with statistics
                                    position = competitor.get('order')
                                    winner = competitor.get('winner', False)

                                    # Fetch competitor statistics for detailed race data
                                    points = 0.0
                                    laps_completed = None
                                    status = None
                                    grid_position = None
                                    ...
                                    if competitor.get('statistics'):
                                        try:
                                            ...
                                            # Extract stats from the splits categories
        ...
        # Use lock for commit to prevent database locked errors
        with db_lock:
            conn.commit()
        return f"✓ {year}"
...
def main():
    parser = argparse.ArgumentParser(description='Complete multithreaded ESPN population')
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        futures = {
            executor.submit(populate_complete_season, year, espn, db_path, db_lock): year
            for year in years
        }
